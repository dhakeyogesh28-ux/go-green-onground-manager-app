...
class _CheckOutScreenState extends State<CheckOutScreen> {
  ...
  /// Validates that all inspection checklist items are completed (either OK or Issue)
  List<String> _validateInspectionChecklist() {
    ...
    _inspectionSections.forEach((section, items) {
      for (var item in items) {
        ...
        // If value is null, it means the item hasn't been checked
        if (value == null) {
  ...
  /// Validates that all required inventory photos are captured
  List<String> _validateRequiredPhotos() {
    final List<String> missingPhotos = [];
    
    for (var category in _photoCategories) {
      final id = category['id'] as String;
      final label = category['label'] as String;
      final photo = _inventoryPhotos[id];
      
      // All photos are required
      if (photo == null) {
        missingPhotos.add(label);
      }
    }
  ...
  Future<void> _handleCheckOut() async {
    ...
    // Validate that all inspection checklist items are completed
    final missingChecklistItems = _validateInspectionChecklist();
    if (missingChecklistItems.isNotEmpty) {
      final itemsList = missingChecklistItems.take(3).join(', ');
      final remaining = missingChecklistItems.length > 3
          ? ' and ${missingChecklistItems.length - 3} more'
          : '';
      
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Please complete all inspection items. Missing: $itemsList$remaining',
          ),
    ...
    final missingPhotos = _validateRequiredPhotos();
    if (missingPhotos.isNotEmpty) {
      final photosList = missingPhotos.take(3).join(', ');
      final remaining = missingPhotos.length > 3
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Please capture all required photos. Missing: $photosList$remaining',
          ),
          backgroundColor: AppTheme.dangerRed,
          duration: const Duration(seconds: 4),
        ),
      ...
      children: [
        ...
        Row(
          children: [
            ...
            const SizedBox(width: 12),
            Expanded(child: _buildPurposeButton('Station Shifting', LucideIcons.truck)),
          ],
        ),
      ],
    );
